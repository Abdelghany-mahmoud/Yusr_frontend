...
function ActivityLog({ client }) {
  const [page, setPage] = useState(1);
  const [isOpen, setIsOpen] = useState(false);
  const { t } = useTranslation("layout");
  const [token] = useRecoilState(tokenAtom);
  const { data, isLoading } = useGetData({
    endpoint: `employees/activity-logs?user_id=${client?.user?.id}&causer_id=${token?.user?.id}&page=${page}`,
    queryKey: ["activity-log", client?.user?.id, page],
    enabledKey: isOpen,
  });
